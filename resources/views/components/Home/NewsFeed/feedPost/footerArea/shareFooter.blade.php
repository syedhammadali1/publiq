<ul class="post-meta-wrap d-flex justify-content-between align-items-center">
    ...
    <li class="post-comment">
        <a class=""><i class="flaticon-comment"></i><span>Comment</span>
            <span class="number post-comment-count-{{ $user_post->id }} ">{{ $user_post->comments_count }}
            </span></a>
    </li>
            ...
            @unless($authUser->id == $user_post->user->id)
                <li>
                    <a class="share-itmes d-flex align-items-center share-post" data-foruse="timeline"
                        data-post_id="{{ $user_post->type == 'text' ? @$user_post->uuid : @$user_post->shareOf->uuid }}">
...
@if ($paginated_comment->lastPage() != 1)
    <div class="more-comments d-flex justify-content-between pt-2">
        <div>
            <a href="#" class="previous-comments pt-2 " data-id="{{ $user_post->uuid }}" data-page="1"
                data-last-page="{{ $paginated_comment->lastPage() }}">Previous Comments+</a>
        </div>
        <a href="#" style="display: none" class="toggle-comments less pt-2" data-id="{{ $user_post->uuid }}">View
            Less</a>

    </div>
@endif
<div class="post-all-comments-{{ $user_post->uuid }} comment-area-scroll">
    @include('components.Home.NewsFeed.comments', [
        'comments' => $paginated_comment,
    ])
</div>
<div class="post-footer m-1">
